...
// Implement sumDataValues method
	public int sumDataValues(Node curNode) {	//method to  sum the data values for each node
		int sum = 0;  //variable to hold the sum of the data values
		while(curNode != null) { //while loop to add each node to the total
			sum = sum + curNode.getData();
			curNode = curNode.getNext();
		}
		return sum;
	}
